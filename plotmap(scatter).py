...
#pop=[200*(x-minpop)/(maxpop-minpop) for x in Lpop]
x,y = map(Llng,Llat)
map.scatter(x,y,s=[1000*(x-minpop)/(maxpop-minpop) for x in Lpop],marker='.',facecolors='r',edgecolors='r')
...
#plt.show()
plt.savefig('Scatter Population Distribution Map')
print('Run geodumpdata.py to choose another year.' )
